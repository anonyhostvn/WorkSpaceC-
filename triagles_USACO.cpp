...
void prepare()
{
    ...
    forinc(j,1,n) if (i != j && a[i].X <= a[j].X) {
        forinc(t,1,n) if (t != j && t != i && a[i].X <= a[t].X && a[t].X <= a[j].X && ccw(a[i],a[j],a[t]) > 0) below[i][j] ++ , dd[i][j][t] = true ;
        below[j][i] = -below[i][j] ;
    }
}
...
void process()
{
    ...
    forinc(j,1,n) if (i != j && a[i].X <= a[j].X)
        forinc(t,1,n) if (t != j && t != i) {
            ...
            if (dd[i][j][t] || dd[i][t][j] || dd[j][t][i]) cnt -- ;
            ans[cnt] ++ ;
        }

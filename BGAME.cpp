...
bool vis[maxN] ;
int f[maxN][25] , L = 0 , g[maxN][25] , stin[maxN] , stout[maxN] , cnt = 0 ;

void dfs(int u , int p) {
    vis[u] = true ; stin[u] = ++cnt ;
    forinc(i,1,20) f[u][i] = f[f[u][i-1]][i-1] , g[u][i] = min(g[u][i-1] , g[f[u][i-1]][i-1]) ;
...
int find_min(int u , int v) {
    int res = ooit ;
    if (u == v) return res ;
    fordec(i,20,0) if (!anc(u , f[v][i]))
        minimize(res , g[v][i]) , v = f[v][i] ;
    minimize(res , f[v][0]) ;
    return res ;
}

int min_from_to(int u , int v) {
    int p = LCA(u,v) ;
    return min (find_min(p,u) , find_min(p,v));
}
...
void process () {
    ...
    forinc(i,1,m) if (!used[i]) {
        int u = Tedge[i].u , v = Tedge[i].v , c = Tedge[i].c ;
        maximize(res , c + min_from_to(u,v)) ;
    }
